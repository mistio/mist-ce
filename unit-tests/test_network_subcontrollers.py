...
def test_0_create_simple_network(cloud):
    ...
    new_network = cloud.ctl.network.create_network(network=network_input)
    db_network = None
    try:
        ...
        db_network = Network.objects.get(network_id=new_network['network']['id'], title='testnet')
        assert isinstance(db_network, Network)
        assert db_network.title == new_network['network']['name']
        assert db_network.network_id == new_network['network']['id']
...
def test_3_delete_network(cloud):
    ...
    db_network = Network.objects.get(network_id=new_network['network']['id'], title='testnettodelete')

    assert cloud.ctl.network.delete_network(db_network.id) is True

    try:
        db_network = Network.objects.get(network__id=new_network['network']['id'], title='testnettodelete')
