    ...
    if provider == 'bare_metal':
        ...
        if provider == 'vcloud':
            for prefix in ['https://', 'http://']:
                cloud.apiurl = cloud.apiurl.strip(prefix)
...
def add_cloud_v_2(user, title, provider, params):
    ...
    if provider == 'bare_metal':
        cloud_id = _add_cloud_bare_metal(user, title, provider, params)
        log.info("Cloud with id '%s' added succesfully.", cloud_id)
        trigger_session_update(user.email, ['clouds'])
        return cloud_id
    elif provider == 'ec2':
...
def _add_cloud_bare_metal(user, title, provider, params):
    ...
    machine.name = machine_hostname
    cloud = model.Cloud()
    cloud.title = title or machine_hostname
    cloud.provider = provider
    cloud.enabled = True
    cloud.machines[machine_id] = machine
    cloud_id = cloud.get_id()
...
def connect_provider(cloud):
    ...
    if cloud.provider == Provider.AZURE:
        ...
        if cloud.apisecret:
            ...
            temp_key_file.write(cloud.apisecret)
            temp_key_file.close()
            conn = driver(cloud.apiurl, user=cloud.apikey, ssh_key=temp_key_file.name, ssh_port=cloud.ssh_port)
        else:
...
def _machine_action(user, cloud_id, machine_id, action, plan_id=None):
...
def list_networks(user, cloud_id):
    ...
    #task_result = task.smart_delay(user.email, cloud_id, blocking=True)
...
def create_network(user, cloud_id, network, subnet):
    """
...
def set_machine_metadata(user, cloud_id, machine_id, tag):
...
def delete_machine_metadata(user, cloud_id, machine_id, tag):
...
def enable_monitoring(user, cloud_id, machine_id,
    ...
    if not no_ssh:
        mist.io.tasks.deploy_collectd.delay(user.email, cloud_id, machine_id,
                                            ret_dict['extra_vars'])
