...
@app.task(bind=True, default_retry_delay=2*60)
def rackspace_first_gen_post_create_steps(self, email, cloud_id, machine_id,
    ...
    def error_rerun_handler(self, exc, errors, email, cloud_id):
        ...
        if index < len(times):
            ...
            with user.lock_n_load():
                ...
                user.save()
            notify_user(user, "Cloud %s djsabled after not responding for 30mins" % user.clouds[cloud_id]['title'],
...
def create_machine_async(email, cloud_id, key_id, machine_name, location_id,
    ...
    specs = []
    for name in names:
        specs.append((user, cloud_id, key_id, name, location_id, image_id,
