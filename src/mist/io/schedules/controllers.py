...
class ScheduleController(object):
    ...
    def add(self, auth_context, **params):
        ...
        # check params
        if not (params.get('script_id', '') or params.get('action', '')):
            raise BadRequestError(
                "You must provide script_id or machine's action")

        if not (params.get('machines_uuids') or params.get('machines_tags')):
            raise BadRequestError(
                "You must provide a list of machine ids or tags")

        try:
            self.update(auth_context, **params)
