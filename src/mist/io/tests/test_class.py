...
class TestClass(unittest.TestCase):
    ...
    def test_01_list_clouds(self):
        ...
        """
        print "\n>>>List of Clouds:"
        self.test_config['BACKENDS'] = clouds.list_clouds(self.uri, cookie=self.cookie) or {}
    ...
    def test_020_add_EC2_cloud(self):
        ...
        for prov in providers:
            if "EC2 AP SOUTHEAST" in prov['title']:
                ...
                self.test_config['BACKENDS'][cloud['id']] = cloud
    ...
    def test_040_list_images(self):
        ...
        """
        for cloud_id in self.test_config['BACKENDS']:
            #cloud_id = self.test_config['BACKENDS'].keys()[0]
            print "\n>>>List of images for Cloud %s" % \
                  self.test_config['BACKENDS'][cloud_id]['title']
            images = clouds.list_images(self.uri, cloud_id, cookie=self.cookie)
            self.test_config['BACKENDS'][cloud_id]['images'] = images
            for image in images:
                print image['name']
    ...
    def test_041_list_sizes(self):
        ...
        for cloud_id in self.test_config['BACKENDS']:
            ...
            self.test_config['BACKENDS'][cloud_id]['sizes'] = sizes
            for size in sizes:
                print size['name']
    ...
    def test_042_list_locations(self):
        ...
        for cloud_id in self.test_config['BACKENDS']:
            ...
            self.test_config['BACKENDS'][cloud_id]['locations'] = locations
    ...
    def test_050_list_machines(self):
        ...
        for cloud_id in self.test_config['BACKENDS']:
            ...
            if mach == {} or mach == []:
                self.test_config['BACKENDS'][cloud_id]['machines'] = {}
            else:
                try:
                    for machine in mach:
                        self.test_config['BACKENDS'][cloud_id]['machines'][machine['name']] = machine
    ...
    def test_051_create_machine_on_EC2_NorthEast(self):
        ...
        while True:
            ...
            if name not in self.test_config['BACKENDS'][cloud_id]['machines']:
                break

        image_id = self.test_config['BACKENDS'][cloud_id]['images'][0]['id']
        size = self.test_config['BACKENDS'][cloud_id]['sizes'][0]['id']
        location = self.test_config['BACKENDS'][cloud_id]['locations'][0]['id']


        print "\n>>>Will now Create Machine:"
        print ">>>>>>>>>>>>>>>>>>>>>>>>>>>>>"
        print "Name: %s" % name
        print "Cloud: %s" % self.test_config['BACKENDS'][cloud_id]['title']
    ...
    def test_052_reboot_machine(self):
        ...
        print "\n>>>Reboot Machine:"
        for cloud_id in self.test_config['BACKENDS']:
            for machine in self.test_config['BACKENDS'][cloud_id]['machines']:
                if self.test_config['MACHINE_NAME'] in machine and \
                        self.test_config['BACKENDS'][cloud_id]['machines'][machine]['can_reboot']:
                    machine_id = self.test_config['BACKENDS'][cloud_id]['machines'][machine]['id']
    ...
    def test_054_start_machine(self):
        ...
        for cloud_id in self.test_config['BACKENDS']:
            for machine in self.test_config['BACKENDS'][cloud_id]['machines']:
                if self.test_config['MACHINE_NAME'] in machine and \
                        self.test_config['BACKENDS'][cloud_id]['machines'][machine]['can_start']:
                    machine_id = self.test_config['BACKENDS'][cloud_id]['machines'][machine]['id']
    ...
    def test_055_destroy_machines(self):
        ...
        for cloud_id in self.test_config['BACKENDS']:
            for machine in self.test_config['BACKENDS'][cloud_id]['machines']:
                if self.test_config['MACHINE_NAME'] in machine and self.test_config['BACKENDS'][cloud_id]['machines'][machine]['can_destroy']:
                    machine_id = self.test_config['BACKENDS'][cloud_id]['machines'][machine]['id']
