...
# Automatically populated mapping of all Network subclasses, keyed by their provider name
NETWORKS = {}
# Automatically populated mapping of all Subnet subclasses, keyed by their provider name
SUBNETS = {}
...
class Network(me.Document):
    """The basic network model. Only meant to be used as a base class for cloud-specific Network subclasses.
    Contains all common provider-independent functionality."""
    ...
    # The 'extra' dictionary returned by libcloud. Contains miscellaneous data about the network object
    extra = me.DictField()
    ...
    def add(cls, cloud, name='', description='', object_id='', **kwargs):
        """Create a new Network DB object and use the appropriate NetworkController to create it on the
        cloud."""
    ...
    def clean(self):
        """Called by Network.save to validate every Field that's persisted to the DB.
        So far, only checks the CIDR address range value to determine if it maps to a valid
        IPv4 address range, if applicable."""
    ...
    def as_dict(self):
        """Returns a representation of the Network object as a JSON-serializable dictionary.
        Used by the list_networks method to return network data to the frontend."""
...
class GoogleNetwork(Network):
    ...
    def clean(self):
        """GCM networks only allow CIDR assignment if the 'legacy' mode has been selected.
        Otherwise, a value of None must be passed to libcloud for the network creation call to succeed."""
        if self.mode == 'legacy':
            super(self, GoogleNetwork).clean()
        else:
            if self.cidr is not None:
                raise me.ValidationError('CIDR cannot be set for modes other than "legacy"')
...
class Subnet(me.Document):
    """The basic subnet model. Only meant to be used as a base class for cloud-specific Subnet subclasses.
        Contains all common provider-independent functionality."""
    id = me.StringField(primary_key=True, default=lambda: uuid.uuid4().hex)
    ...
    """The network this subnet is attached to. This Subnet object will automatically be deleted if the network
    is deleted."""
    network = me.ReferenceField('Network', required=True, reverse_delete_rule=me.CASCADE)
    description = me.StringField()

    # The 'extra' dictionary returned by libcloud. Contains miscellaneous data about the subnet object
    extra = me.DictField()
    ...
    def add(cls, network, cidr, name='', description='', object_id='', **kwargs):
        """Create a new Subnet DB object and use the appropriate NetworkController to create it on the
                cloud."""
